...
if (isset($_GET['propertyID'])) {
    ...
    $sql = "SELECT gp.*, ap.AREno, agp.dateReceived, agp.soQty, agp.soValue, agp.previousCondition,
    ...
    if ($result->num_rows > 0) {
        ...
        header("Location: inactivePPE.php");
        exit();
    }
}
...
if (isset($_POST['btn_updateARE'])) {
    ...
    if (isset($_FILES['scannedDocs']) && $_FILES['scannedDocs']['name'] !== '') {
        ...
        $file_destination = './ARE SCANNED DOCUMENTS/' . $new_file_name;
    ...
    if ($stmt_generalproperties && $stmt_are_ics_gen_properties && $stmt_are_properties) {
        ...
        if ($stmt->execute()) {
            ...
            echo '<div class="modal-message">Updated Successfully</div>';
            echo '<button class="ok-button" onclick="window.location.href=\'inactivePPE.php\'">OK</button>';
            echo '</div>';
            echo '</div>';
        } else {
