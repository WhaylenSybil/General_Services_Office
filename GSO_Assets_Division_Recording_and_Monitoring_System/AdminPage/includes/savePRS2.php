...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $stmtGen->bind_param("ssssssssssssssssss",
        $article, 
        $brand, 
        $serialNo, 
        $particulars, 
                ...
                if ($stmtPrsProperties->execute()) {
                    ...
                    if ($stmtPRS->execute()) {
                        ...
                        if ($stmtModeOfDisposal->execute()) {
                            ...
                            $status = isset($_POST['updatesCurrentStatus']) ? $_POST['updatesCurrentStatus'] : '';

                            if ($status === "Dropped In Both Records") {
                                $jevNoDropped = isset($_POST['jevNoDropped']) ? $_POST['jevNoDropped'] : '';
                                $dateDropped = isset($_POST['dateDropped']) ? $_POST['dateDropped'] : '';
                                $actionsToBeTaken = isset($_POST['notesDropped']) ? $_POST['notesDropped'] : '';

                                // Insert data into updates_or_status table
                                $stmtCurrentStatus = $connect->prepare("INSERT INTO updates_or_status (prsID, currentStatus, jevNoDropped, dateDropped, actionsToBeTaken) VALUES (?, ?, ?, ?, ?)");
                                $stmtCurrentStatus->bind_param("issss", $prsID, $status, $jevNoDropped, $dateDropped, $actionsToBeTaken);
                            }
                            elseif ($status === "Existing In Inventory Report") {
                                $actionsToBeTaken = isset($_POST['remarksExisting']) ? $_POST['remarksExisting'] : '';

                                // Insert data into updates_or_status table
                                $stmtCurrentStatus = $connect->prepare("INSERT INTO updates_or_status (prsID, currentStatus, actionsToBeTaken) VALUES (?, ?, ?)");
                                $stmtCurrentStatus->bind_param("iss", $prsID, $status, $actionsToBeTaken);
                            ...
                            displayModalWithRedirect("Added a PRS", "PRS.php");
                                ...
                                echo "Error:" .$stmtCurrentStatus->error;
                            }
                            $stmtCurrentStatus->close();
                        } else {
