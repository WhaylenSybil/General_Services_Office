    ...
    if ($stmtGen->execute()) {
        ...
        if ($stmtAre->execute()) {
            ...
            if ($stmtAreProperties->execute()) {
                ...
                $withAttachment = isset($_POST['withAttachment']) ? $_POST['withAttachment'] : '';
                $withCoverPage = isset($_POST['withCoverPage']) ? $_POST['withCoverPage'] : '';
                ...
                if ($stmtPrsProperties->execute()) {
                    ...
                    if ($stmtPRS->execute()) {
                        ...
                        $stmtModeOfDisposal = $connect->prepare("INSERT INTO mode_of_disposal (prsID, modeOfDisposal, partDestroyed, dateOfSale, dateOfAuction, transferDate, recipient, ORdate, ORnumber, amount, modeRemarks)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
