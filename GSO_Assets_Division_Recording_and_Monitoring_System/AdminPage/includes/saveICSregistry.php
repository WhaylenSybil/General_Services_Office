...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['scannedDocs']) && $_FILES['scannedDocs']['error'] == 0) {
        ...
        $ICSno = $_POST['ICSno'][0]; // Assuming it's a single value, adjust as needed
    ...
    $ICSnos = isset($_POST["ICSno"]) ? $_POST["ICSno"] : [];
    ...
    foreach ($accountablePersons as $key => $accountablePerson) {
        ...
        $insertAREPropertiesQuery = "INSERT INTO ics_properties (propertyID, ARE_ICS_id, ICSno) VALUES (?, ?, ?)";
        $stmt = $connect->prepare($insertAREPropertiesQuery);
        $stmt->bind_param("iis", $propertyID, $ARE_ICS_id, $ICSnos[$key]);
        $stmt->execute();
        $stmt->close();
        ...
        displayModalWithRedirect("Added an ICS Property", "activeSemiPPE.php");
    }/*foreach*/
